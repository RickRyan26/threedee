...
export const shiftyWalls = Array(Math.floor(trackLength / 100)).fill('x').map((_, i) => {
		return {
			position: {
...
export const shiftyBars = Array(Math.floor(trackLength / 100)).fill('x').map((_, i) => {
...
export const selectRigidBodyPos = (axis) => {
...
function updateMovement(action, status){
...
*/
export function handleKeys(evt, status){
		switch(evt.code){
			case "Space":
